...
- **자동 오래된 작업 감지 및 재시작**: **Celery 워커가 시작될 때 자동으로** 다음 작업을 수행합니다:
  1. **오래된 작업 실패 처리**: 18분 이상 지난 "처리 중" 작업을 감지하고 실패로 표시
  2. **멈춘 작업 재시작**: 5분 이상 지난 "처리 중" 작업을 재시작 (실패 상태인 작업은 재시작하지 않음)
  3. **상태 재확인**: 재시작 전에 데이터베이스에서 최신 상태를 다시 확인하여 실패로 변경된 작업은 재시작하지 않음
...
2. 5분 이상 지난 "처리 중" 작업을 재시작 (실패 상태 제외)

별도의 설정이나 명령어 실행이 필요 없습니다. 실패로 변경된 작업은 재시작되지 않아 중복 처리나 무한 루프를 방지합니다.
