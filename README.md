...
- **자동 오래된 작업 감지**: **Celery 워커가 시작될 때 자동으로** 오래된 "처리 중" 작업을 감지하고 실패로 표시 (15분 이상 지난 작업)
- **수동 오래된 작업 감지**: 관리 명령어를 통해 수동으로 오래된 작업을 감지할 수 있음

#### 자동 감지
Celery 워커를 시작하면 (`celery -A config worker -l info`) 자동으로 15분 이상 지난 "처리 중" 작업을 체크하고 실패로 표시합니다. 별도의 설정이나 명령어 실행이 필요 없습니다.
