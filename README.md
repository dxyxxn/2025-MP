...
- **오디오 파일 업로드** 또는 **YouTube URL 입력** (음성 강의)
  ...
  - YouTube URL: yt-dlp를 사용하여 오디오만 자동 다운로드 (10분 타임아웃)
- PDF 파일 업로드 (강의 자료)
...
- **자동 오래된 작업 감지**: **Celery 워커가 시작될 때 자동으로** 오래된 "처리 중" 작업을 감지하고 실패로 표시 (18분 이상 지난 작업)
- **수동 오래된 작업 감지**: 관리 명령어를 통해 수동으로 오래된 작업을 감지할 수 있음

#### 자동 감지
Celery 워커를 시작하면 (`celery -A config worker -l info`) 자동으로 18분 이상 지난 "처리 중" 작업을 체크하고 실패로 표시합니다. 별도의 설정이나 명령어 실행이 필요 없습니다.
...
- **Ollama 타임아웃 및 재시도**: PDF 이미지 분석 시 타임아웃 및 자동 재시도로 안정성 향상
  - 개별 이미지 분석: 타임아웃 및 재시도 지원
  - 배치 처리: 배치 전체 실패 시 자동 재시도 (최대 재시도 횟수 설정 가능)
