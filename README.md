...
### 3. 자동 처리 파이프라인
강의 파일이 업로드되면 다음 6단계가 자동으로 실행됩니다:

#### 단계 1: STT 처리
- 오디오 파일을 Gemini API를 통해 텍스트로 변환 (타임스탬프 포함)
- 각 단계 완료 시 소요 시간 표시

#### 단계 2: PDF 파싱 및 텍스트 추출
- **하이브리드 방식**: PyMuPDF로 페이지 텍스트를 정확하고 빠르게 추출
- 페이지에서 이미지 객체 추출
- 각 이미지를 Ollama bakllava 모델로 분석 (영어 설명)
- 텍스트와 이미지 설명을 결합하여 완전한 페이지 내용 생성
- 각 단계 완료 시 소요 시간 표시

#### 단계 3: 스크립트 요약 생성
- 전체 스크립트를 Gemini API로 분석하여 소주제별로 구조화된 요약 생성
...
Celery 백그라운드 작업 시작
    ├─ 1단계: STT 처리 (오디오 → 텍스트)
    ├─ 2단계: PDF 파싱 및 텍스트 추출 (PyMuPDF + Ollama 이미지 분석)
    ├─ 3단계: 요약 생성 (소주제별 구조화)
    ├─ 4단계: 임베딩 생성 및 ChromaDB 저장
    ├─ 5단계: 의미 기반 매핑 (요약 ↔ PDF 페이지)
    ├─ 6단계: 데이터 저장
...
- **Lecture**: 강의 메타데이터 및 처리 상태
  - `current_step`: 현재 처리 단계 (0~6)
  - `step_times`: 단계별 소요 시간 (JSON 형식)
  - `estimated_time_sec`: 예상 소요 시간(초)
...
- **실시간 진행 상황**: 처리 중 페이지에서 6단계별 진행률 및 소요 시간 표시
